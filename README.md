...
Sistema web de gestión clínica de vacunación infantil desarrollado con Flask y PostgreSQL, con una arquitectura de tres capas (presentación, lógica de negocio y datos).
...
5. [Restauración de la Base de Datos](#5-restauración-de-la-base-de-datos)
6. [Configuración del Entorno Virtual Python](#6-configuración-del-entorno-virtual-python)
7. [Instalación de Dependencias Python](#7-instalación-de-dependencias-python)
8. [Configuración de Conexión a Base de Datos](#8-configuración-de-conexión-a-base-de-datos)
...
- **Base de datos:** PostgreSQL (con stored procedures, triggers y vistas)
...
> Reemplaza `tu_contraseña_segura` por una contraseña de tu elección. Guárdala, la necesitarás en el paso 8.
...
## 6. Configuración del Entorno Virtual Python
...
El sistema obtiene los parámetros de conexión a PostgreSQL desde el archivo de configuración de la aplicación. Busca en el proyecto el archivo donde se define la cadena de conexión (puede llamarse `config.py`, `db.py`, `database.py` o estar directamente en `app_2daE.py`) y ajusta los siguientes valores:
...
```python
DATABASE_URL = "postgresql://postgres:tu_contraseña_segura@localhost:5432/sistemavacunacion"
```

> Asegúrate de que los valores coincidan exactamente con los configurados en PostgreSQL.

---

## 9. Ejecución del Sistema

### 9.1 Definir el punto de entrada de Flask

```bash
export FLASK_APP=app_2daE.py
```

### 9.2 (Opcional) Activar el modo de desarrollo

```bash
export FLASK_ENV=development
```

### 9.3 Iniciar el servidor
...
├── sistemavacunacionBACKUP3.sql    # Respaldo completo de la base de datos
...
├── models/                         # Acceso y consulta a la base de datos
└── ...
```
...
### Tras restaurar la base de datos
...
- No distribuyas archivos `.sql` de respaldo con datos sensibles de producción.
...
- Considera usar variables de entorno (`.env` + `python-dotenv`) para manejar credenciales en lugar de escribirlas directamente en el código.
