...
\restrict MTI0IbsfvR6mBlr5ySsyB4RUWlksL94H0dzAoSoIuhE6fRq8cjQdyfp8lns3uY2
...
--
-- Name: sp_assign_nfc_card(integer, character varying, character varying, integer, boolean, text, refcursor); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.sp_assign_nfc_card(IN p_patient_id integer, IN p_uid character varying, IN p_card_type character varying, IN p_issued_by integer, IN p_force boolean, IN p_notes text, INOUT p_results refcursor)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_nfc_card_id INT;
BEGIN
    ...
    IF EXISTS (
        ...
        IF NOT COALESCE(p_force, FALSE) THEN
            RAISE EXCEPTION 'El paciente ya tiene una tarjeta NFC activa. Desact¡vala antes de asignar una nueva.';
        ELSE
            UPDATE nfc_cards
            SET status = 'Inactiva',
                nfc_card_notes = COALESCE(nfc_card_notes, '') || ' - Desactivada por reasignacion forzada'
...
ALTER PROCEDURE public.sp_assign_nfc_card(IN p_patient_id integer, IN p_uid character varying, IN p_card_type character varying, IN p_issued_by integer, IN p_force boolean, IN p_notes text, INOUT p_results refcursor) OWNER TO postgres;
...
--
-- Name: sp_clear_patient_nfc_id(integer); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.sp_clear_patient_nfc_id(IN p_patient_id integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE patients
...
ALTER PROCEDURE public.sp_clear_patient_nfc_id(IN p_patient_id integer) OWNER TO postgres;
...
--
-- Name: sp_nfc_checkout(character varying, integer, character varying, integer, refcursor); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.sp_nfc_checkout(IN p_nfc_uid character varying, IN p_worker_id integer, IN p_device_id character varying, IN p_clinic_id integer, INOUT p_result refcursor)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_card_id      INT;
...
BEGIN
    ...
    IF NOT FOUND THEN
        OPEN p_result FOR
            SELECT FALSE AS success, 'NFC no v lido o inactivo' AS message,
...
ALTER PROCEDURE public.sp_nfc_checkout(IN p_nfc_uid character varying, IN p_worker_id integer, IN p_device_id character varying, IN p_clinic_id integer, INOUT p_result refcursor) OWNER TO postgres;

--
-- Name: sp_nfc_medical_scan(character varying, integer, character varying, integer, refcursor); Type: PROCEDURE; Schema: public; Owner: postgres
--

CREATE PROCEDURE public.sp_nfc_medical_scan(IN p_nfc_uid character varying, IN p_worker_id integer, IN p_device_id character varying, IN p_clinic_id integer, INOUT p_result refcursor)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_card_id        INT;
...
BEGIN
    ...
    -- Si el paciente est  En espera, transicionar a En consulta autom ticamente
    IF v_visit_id IS NOT NULL AND v_visit_status = 'En espera' THEN
        ...
        VALUES (
            ...
            NOW(), p_worker_id, 'NFC m‚dico - inicia consulta'
        );
...
ALTER PROCEDURE public.sp_nfc_medical_scan(IN p_nfc_uid character varying, IN p_worker_id integer, IN p_device_id character varying, IN p_clinic_id integer, INOUT p_result refcursor) OWNER TO postgres;
...
BEGIN
    OPEN p_results FOR
        ...
        FROM (
            -- Citas agendadas en las £ltimas 24 h
            SELECT
            ...
            -- Pacientes registrados en las £ltimas 24 h
            SELECT
...
BEGIN
    OPEN p_results FOR
        SELECT
            ...
            -- Alerta: hora ya pas¢ y a£n no se registr¢ asistencia
            (
                a.scheduled_at < NOW()
...
BEGIN
    ...
    OPEN p_results FOR
        SELECT
            ...
            -- Pr¢ximas 3 dosis pendientes del paciente (JSON)
            (
                SELECT JSON_AGG(sub)
                FROM (
                    SELECT
...
BEGIN
    ...
    -- ÄÄ Lotes pr¢ximos a vencer (=30 d¡as con stock disponible) ÄÄÄÄÄÄÄÄÄÄÄÄÄ
    ...
    -- ÄÄ Alertas de esquema pendientes (no resueltas ni le¡das) ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
    BEGIN
    ...
    -- ÄÄ Retraso promedio (d¡as entre due_date y applied_date) ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
...
--
-- Name: v_reportes_scheme_delay; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_reportes_scheme_delay AS
 SELECT base.patient_id,
    base.applied_date,
    pvs.due_date
   FROM (public.v_patient_vaccination_scheme_base base
     JOIN public.patient_vaccine_schedule pvs ON ((pvs.schedule_id = base.schedule_id)))
  WHERE ((base.applied_date IS NOT NULL) AND (base.applied_date > pvs.due_date));


ALTER VIEW public.v_reportes_scheme_delay OWNER TO postgres;

--
-- Name: v_reportes_vaccination_geo; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_reportes_vaccination_geo AS
 SELECT vr.record_id,
    vr.patient_id,
    vr.worker_id,
    vr.vaccine_id,
   ...
   FROM (((((public.vaccination_records vr
     ...
     JOIN public.addresses a ON ((a.address_id = c.address_id)))
     JOIN public.neighborhoods n ON ((n.neighborhood_id = a.neighborhood_id)))
     JOIN public.municipalities m ON ((m.municipality_id = n.municipality_id)));


ALTER VIEW public.v_reportes_vaccination_geo OWNER TO postgres;

--
-- Name: v_scheme_alerts_full; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_scheme_alerts_full AS
 SELECT sca.alert_id,
    sca.schedule_id,
    sca.alert_type,
    ...
    pvs.due_date,
    pvs.status AS dose_status,
    TRIM(BOTH FROM (((p.first_name)::text || ' '::text) || (p.last_name)::text)) AS patient_name,
    sd.dose_label,
    v.name AS vaccine_name
   FROM ((((public.scheme_completion_alerts sca
     JOIN public.patient_vaccine_schedule pvs ON ((pvs.schedule_id = sca.schedule_id)))
     JOIN public.patients p ON ((p.patient_id = pvs.patient_id)))
     JOIN public.scheme_doses sd ON ((sd.dose_id = pvs.scheme_dose_id)))
     JOIN public.vaccines v ON ((v.vaccine_id = sd.vaccine_id)));


ALTER VIEW public.v_scheme_alerts_full OWNER TO postgres;
...
--
-- Name: v_vaccine_lots_detail; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.v_vaccine_lots_detail AS
 SELECT vl.lot_id,
    vl.vaccine_id,
    vl.clinic_id,
    vl.quantity_available,
    ...
    ((vl.expiration_date >= CURRENT_DATE) AND (vl.expiration_date <= (CURRENT_DATE + 30))) AS is_expiring_soon,
    v.name AS vaccine_name,
    c.name AS clinic_name
   FROM ((public.vaccine_lots vl
     JOIN public.vaccines v ON ((v.vaccine_id = vl.vaccine_id)))
     JOIN public.clinics c ON ((c.clinic_id = vl.clinic_id)));


ALTER VIEW public.v_vaccine_lots_detail OWNER TO postgres;
...
CREATE VIEW public.vw_patients AS
 SELECT p.patient_id,
    ...
    p.created_at,
    p.blood_type_id,
    COALESCE(bt.blood_type, '-'::character varying) AS blood_type,
    (date_part('year'::text, age((CURRENT_DATE)::timestamp with time zone, (p.birth_date)::timestamp with time zone)))::integer AS age,
    g.guardian_id,
    COALESCE(TRIM(BOTH FROM (((g.first_name)::text || ' '::text) || (g.last_name)::text)), 'Sin tutor'::text) AS guardian,
    COALESCE(( SELECT gp.phone
           FROM public.guardian_phones gp
          WHERE (gp.guardian_id = g.guardian_id)
          ORDER BY gp.is_primary DESC, gp.phone_id
         LIMIT 1), '-'::character varying) AS contact,
    COALESCE(NULLIF(( SELECT string_agg((al.name)::text, ', '::text ORDER BY (al.name)::text) AS string_agg
           FROM (public.patient_allergies pa
             JOIN public.allergies al ON ((al.allergy_id = pa.allergy_id)))
          WHERE (pa.patient_id = p.patient_id)), ''::text), 'Ninguna'::text) AS allergies,
    'N/A'::text AS risk
   FROM (((public.patients p
     LEFT JOIN public.blood_types bt ON ((bt.blood_type_id = p.blood_type_id)))
     LEFT JOIN LATERAL ( SELECT pgr.guardian_id
           FROM public.patient_guardian_relations pgr
          WHERE (pgr.patient_id = p.patient_id)
          ORDER BY pgr.is_primary DESC, pgr.relation_id
         LIMIT 1) rel ON (true))
     LEFT JOIN public.guardians g ON ((g.guardian_id = rel.guardian_id)));
...
16	1	3	3	2026-05-18 08:00:00	20	\N	No Show	[2026-05-19] Paciente no asisti¢.	2026-05-17 01:49:40.170568	444	\N	\N	\N	Administrador	1	\N	18
25	1	2	3	2026-05-19 13:30:00	20	\N	Cancelada	[2026-05-19] Cancelada. Motivo: Cancelada por personal clínico	2026-05-19 10:18:26.629835	10	\N	Cancelada por personal clínico	\N	Medico	3	\N	10
23	1	2	3	2026-05-19 12:30:00	20	\N	Cancelada	[2026-05-19] Cancelada. Motivo: Cancelada por tutor	2026-05-19 10:16:43.642264	76	\N	Cancelada por tutor	\N	Medico	3	\N	1
21	1	3	2	2026-05-18 09:30:00	20	\N	Cancelada	[2026-05-19] Cancelada. Motivo: Cancelada por personal clínico	2026-05-17 13:36:56.157824	16	\N	Cancelada por personal clínico	\N	Enfermero	2	\N	1
26	1	2	3	2026-06-08 08:00:00	20	\N	Programada	\N	2026-05-19 10:38:45.725441	550	\N	\N	\N	Administrador	1	\N	22
27	1	\N	\N	2026-05-20 08:00:00	20	\N	Programada	\N	2026-05-19 12:42:23.764948	600	\N	\N	\N	Tutor	\N	21	24
28	1	\N	\N	2026-05-20 08:00:00	20	\N	Programada	\N	2026-05-19 12:45:07.848169	106	\N	\N	\N	Tutor	\N	1	1
29	1	2	2	2026-05-26 08:00:00	20	\N	Programada	\N	2026-05-19 14:20:28.058302	31	\N	\N	\N	Tutor	\N	1	1
30	1	2	3	2026-05-21 09:00:00	20	\N	Programada	\N	2026-05-19 14:21:51.846427	604	\N	\N	\N	Tutor	\N	21	24
31	1	2	3	2026-05-26 09:00:00	20	\N	Programada	\N	2026-05-19 15:00:37.29814	630	\N	\N	\N	Medico	3	\N	25
24	1	2	3	2026-05-19 13:00:00	20	\N	Completada	\N	2026-05-19 10:17:27.724583	34	\N	\N	\N	Medico	3	\N	4
22	1	2	3	2026-05-19 12:00:00	20	\N	No Show	[2026-05-19] Paciente no asisti¢.	2026-05-19 10:04:52.625974	41	\N	\N	\N	Recepcionista	4	\N	11
32	1	2	3	2026-05-26 11:00:00	20	\N	Programada	\N	2026-05-19 15:20:51.488039	656	\N	\N	\N	Medico	3	\N	26
33	1	2	2	2026-06-01 13:00:00	20	\N	Programada	\N	2026-05-19 15:24:35.460804	653	\N	\N	\N	Tutor	\N	23	26
\.
...
COPY public.audit_log (audit_id, table_name, record_id, action, worker_id, changed_at, ip_address, changed_data) FROM stdin;
2	vaccination_records	1	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
3	vaccination_records	2	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
4	vaccination_records	3	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
5	vaccination_records	4	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
6	vaccination_records	5	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
7	vaccination_records	6	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
8	vaccination_records	7	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
9	vaccination_records	8	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
10	vaccination_records	9	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
11	vaccination_records	10	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
12	vaccination_records	11	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
13	vaccination_records	12	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
14	vaccination_records	13	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
15	vaccination_records	14	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
16	vaccination_records	15	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
17	vaccination_records	16	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
18	vaccination_records	17	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
19	vaccination_records	18	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
20	vaccination_records	19	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
21	vaccination_records	20	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
22	vaccination_records	21	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
23	vaccination_records	22	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
24	vaccination_records	23	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
25	vaccination_records	24	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
26	vaccination_records	25	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
27	vaccination_records	26	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
28	vaccination_records	27	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
29	vaccination_records	28	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
30	vaccination_records	29	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
31	vaccination_records	30	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
32	vaccination_records	31	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
33	vaccination_records	32	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
34	vaccination_records	33	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
35	vaccination_records	34	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
36	vaccination_records	35	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
37	vaccination_records	36	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
38	vaccination_records	37	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
39	vaccination_records	38	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
40	vaccination_records	39	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
41	vaccination_records	40	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
42	vaccination_records	41	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
43	vaccination_records	42	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
44	vaccination_records	43	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
45	vaccination_records	44	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
46	vaccination_records	45	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
47	vaccination_records	46	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
48	vaccination_records	47	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
49	vaccination_records	48	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
50	vaccination_records	49	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
51	vaccination_records	50	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
52	vaccination_records	51	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
53	vaccination_records	52	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
54	vaccination_records	53	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
55	vaccination_records	54	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
56	vaccination_records	55	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
57	vaccination_records	56	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
58	vaccination_records	57	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
59	vaccination_records	58	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
60	vaccination_records	59	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
61	vaccination_records	60	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
62	vaccination_records	61	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
63	vaccination_records	62	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
64	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
65	vaccination_records	63	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
66	vaccination_records	64	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
67	vaccination_records	65	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
68	vaccination_records	66	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
69	vaccination_records	67	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
70	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
71	vaccination_records	68	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
72	vaccination_records	69	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
73	vaccination_records	70	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
74	vaccination_records	71	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
75	vaccination_records	72	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
76	vaccination_records	73	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
77	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
78	vaccination_records	74	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
79	vaccination_records	75	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
80	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
81	vaccination_records	76	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
82	vaccination_records	77	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
83	vaccination_records	78	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
84	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
85	vaccination_records	79	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
86	vaccination_records	80	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
87	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
88	vaccination_records	81	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
89	vaccination_records	82	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
90	vaccination_records	83	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
91	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
92	vaccination_records	84	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
93	vaccination_records	85	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
94	vaccination_records	86	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
95	vaccination_records	87	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
96	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
97	vaccination_records	88	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
98	vaccination_records	89	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
99	vaccination_records	90	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
100	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
101	vaccination_records	91	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
102	vaccination_records	92	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
103	vaccination_records	93	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
104	vaccination_records	94	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
105	vaccine_lots	8	UPDATE	\N	2026-05-16 17:25:31.438879	\N	\N
106	vaccination_records	95	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
107	vaccination_records	96	INSERT	\N	2026-05-16 17:25:31.438879	\N	\N
108	workers	1	UPDATE	\N	2026-05-16 21:20:31.770927	\N	\N
109	workers	2	UPDATE	\N	2026-05-16 21:20:31.867408	\N	\N
110	workers	7	UPDATE	\N	2026-05-16 21:20:31.869825	\N	\N
111	workers	9	UPDATE	\N	2026-05-16 21:20:31.871682	\N	\N
112	workers	10	UPDATE	\N	2026-05-16 21:20:31.877963	\N	\N
113	workers	11	UPDATE	\N	2026-05-16 21:20:31.879947	\N	\N
114	workers	12	UPDATE	\N	2026-05-16 21:20:33.73647	\N	\N
115	patients	16	INSERT	\N	2026-05-17 00:21:21.918684	\N	\N
116	patients	17	INSERT	\N	2026-05-17 00:28:03.399482	\N	\N
117	patients	16	UPDATE	\N	2026-05-17 00:31:35.731314	\N	\N
118	patients	17	UPDATE	\N	2026-05-17 00:31:41.345002	\N	\N
119	patients	18	INSERT	\N	2026-05-17 00:33:11.833123	\N	\N
120	vaccination_records	97	INSERT	\N	2026-05-17 00:44:12.063684	\N	\N
121	patients	19	INSERT	\N	2026-05-17 13:21:11.16144	\N	\N
123	vaccine_lots	10	UPDATE	\N	2026-05-17 16:58:44.19143	\N	\N
124	vaccination_records	98	INSERT	\N	2026-05-17 19:37:17.485671	\N	\N
125	vaccination_records	99	INSERT	\N	2026-05-17 19:38:57.474474	\N	\N
126	patients	15	UPDATE	\N	2026-05-18 13:13:56.243863	\N	\N
127	patients	6	UPDATE	\N	2026-05-18 13:15:44.971521	\N	\N
128	patients	20	INSERT	\N	2026-05-18 19:43:45.972245	\N	\N
129	patients	21	INSERT	\N	2026-05-19 08:40:14.335041	\N	\N
130	patients	21	UPDATE	\N	2026-05-19 08:44:11.844922	\N	\N
133	patient_clinic_visits	1	INSERT	1	2026-05-19 09:35:38.427308	\N	{"action": "checkin", "clinic_id": 1, "worker_id": 1, "patient_id": 21, "visit_type": "Espontanea"}
134	patient_clinic_visits	1	UPDATE	1	2026-05-19 09:37:27.988028	\N	{"to_status": "En consulta", "worker_id": 1, "from_status": "En espera"}
135	patient_clinic_visits	1	UPDATE	1	2026-05-19 09:37:41.160295	\N	{"action": "checkout", "patient_id": 21, "from_status": "En consulta", "duration_minutes": 2}
136	patients	21	UPDATE	\N	2026-05-19 09:50:41.533794	\N	\N
137	patients	8	UPDATE	\N	2026-05-19 09:51:05.806558	\N	\N
139	patients	8	UPDATE	\N	2026-05-19 09:53:26.727749	\N	\N
140	patients	8	UPDATE	\N	2026-05-19 09:53:55.3046	\N	\N
141	patients	20	UPDATE	\N	2026-05-19 09:54:49.86596	\N	\N
142	patients	12	UPDATE	\N	2026-05-19 09:55:57.259419	\N	\N
143	patients	18	UPDATE	\N	2026-05-19 09:56:15.183623	\N	\N
144	patients	13	UPDATE	\N	2026-05-19 09:56:53.870464	\N	\N
145	patients	9	UPDATE	\N	2026-05-19 09:57:02.716929	\N	\N
146	patients	9	UPDATE	\N	2026-05-19 09:57:16.204341	\N	\N
147	patients	5	UPDATE	\N	2026-05-19 09:57:34.621227	\N	\N
148	patients	11	UPDATE	\N	2026-05-19 09:58:03.703451	\N	\N
149	patients	10	UPDATE	\N	2026-05-19 09:58:20.267476	\N	\N
150	patients	22	INSERT	\N	2026-05-19 10:32:38.871803	\N	\N
151	patients	22	UPDATE	\N	2026-05-19 10:32:38.945606	\N	\N
152	vaccination_records	100	INSERT	\N	2026-05-19 10:35:47.723933	\N	\N
153	vaccination_records	101	INSERT	\N	2026-05-19 10:36:22.850544	\N	\N
154	vaccine_lots	15	UPDATE	\N	2026-05-19 10:36:22.850544	\N	\N
155	patients	22	UPDATE	\N	2026-05-19 10:40:01.230645	\N	\N
156	vaccination_records	102	INSERT	\N	2026-05-19 12:08:35.074615	\N	\N
157	vaccine_lots	15	UPDATE	\N	2026-05-19 12:08:35.074615	\N	\N
158	vaccination_records	103	INSERT	\N	2026-05-19 12:09:03.389673	\N	\N
159	vaccination_records	104	INSERT	\N	2026-05-19 12:10:33.376225	\N	\N
160	vaccination_records	105	INSERT	\N	2026-05-19 12:11:42.3566	\N	\N
161	patients	23	INSERT	\N	2026-05-19 12:30:11.343401	\N	\N
162	patients	23	UPDATE	\N	2026-05-19 12:30:11.456944	\N	\N
163	patients	23	UPDATE	\N	2026-05-19 12:30:55.985843	\N	\N
165	patients	24	INSERT	\N	2026-05-19 12:34:16.998522	\N	\N
166	patients	24	UPDATE	\N	2026-05-19 12:34:17.088947	\N	\N
167	vaccination_records	106	INSERT	\N	2026-05-19 12:36:54.693145	\N	\N
168	vaccination_records	107	INSERT	\N	2026-05-19 12:37:22.933707	\N	\N
169	patient_clinic_visits	2	INSERT	1	2026-05-19 12:46:05.98419	\N	{"action": "checkin", "clinic_id": 1, "worker_id": 1, "patient_id": 24, "visit_type": "Espontanea"}
170	patient_clinic_visits	2	UPDATE	1	2026-05-19 12:46:21.387893	\N	{"to_status": "En consulta", "worker_id": 1, "from_status": "En espera"}
171	patient_clinic_visits	2	UPDATE	1	2026-05-19 12:46:33.607594	\N	{"action": "checkout", "patient_id": 24, "from_status": "En consulta", "duration_minutes": 0}
172	patients	25	INSERT	\N	2026-05-19 14:54:26.19645	\N	\N
173	patients	25	UPDATE	\N	2026-05-19 14:54:47.208731	\N	\N
174	vaccination_records	108	INSERT	\N	2026-05-19 14:56:27.638907	\N	\N
175	vaccination_records	109	INSERT	\N	2026-05-19 14:57:11.73766	\N	\N
176	vaccine_lots	15	UPDATE	\N	2026-05-19 14:57:11.73766	\N	\N
177	vaccination_records	110	INSERT	\N	2026-05-19 15:01:53.26926	\N	\N
178	vaccination_records	111	INSERT	\N	2026-05-19 15:02:37.342834	\N	\N
179	patients	23	UPDATE	\N	2026-05-19 15:06:37.766272	\N	\N
180	patients	24	UPDATE	\N	2026-05-19 15:06:48.589278	\N	\N
181	patients	26	INSERT	\N	2026-05-19 15:12:36.131014	\N	\N
182	patients	26	UPDATE	\N	2026-05-19 15:12:36.226361	\N	\N
183	patients	26	UPDATE	\N	2026-05-19 15:13:12.845721	\N	\N
184	vaccination_records	112	INSERT	\N	2026-05-19 15:15:02.959674	\N	\N
185	vaccination_records	113	INSERT	\N	2026-05-19 15:15:53.638881	\N	\N
186	vaccination_records	114	INSERT	\N	2026-05-19 15:16:41.357646	\N	\N
187	vaccination_records	115	INSERT	\N	2026-05-19 15:17:03.408446	\N	\N
188	patients	27	INSERT	\N	2026-05-19 15:26:49.209163	\N	\N
189	patient_clinic_visits	3	INSERT	4	2026-05-19 15:28:20.037188	\N	{"action": "checkin", "clinic_id": 1, "worker_id": 4, "patient_id": 26, "visit_type": "Espontanea"}
190	patient_clinic_visits	3	UPDATE	4	2026-05-19 15:28:30.466336	\N	{"to_status": "En consulta", "worker_id": 4, "from_status": "En espera"}
191	patient_clinic_visits	3	UPDATE	4	2026-05-19 15:28:40.717508	\N	{"action": "checkout", "patient_id": 26, "from_status": "En consulta", "duration_minutes": 0}
192	patients	26	UPDATE	\N	2026-05-19 15:43:12.784323	\N	\N
194	patients	26	UPDATE	\N	2026-05-19 15:43:47.578957	\N	\N
196	patients	26	UPDATE	\N	2026-05-19 15:44:11.785654	\N	\N
\.
...
4	13	ricardo.morales@gmail.com	$2a$06$T2d0crvDlINzjW5vFkXxFu88rMbyZieB42u2RDedCk2GqF5xBY4tO	t	t	\N	2026-05-19 12:01:19.89258
5	21	rodrigo.figueroa@gmail.com	$2a$06$bEYFasRRuJvlZyVb4kOwEuWyreCV9T6ToQajmoBWQ/myqrYwOZqY6	t	t	\N	2026-05-19 12:41:07.575733
6	23	luis.bonavena@gmail.com	$2a$06$ysi89uPTbOTq/FspY0MCW.AyotM0zIx1P/xnd41QoDVhhj/RY0iUu	t	t	\N	2026-05-19 15:21:53.52529
7	2	maria.martinez@gmail.com	$2a$06$O/6wHpg8SZ/bPLKlcVGSe.uKF/8mMbHdGVsvrxDuQ8Y1Q/D/I81my	t	t	\N	2026-05-19 16:19:50.612412
8	3	luis.lopez@gmail.com	$2a$06$bypq72GfeayY/mZyaHKVHOLFLLJGCrP.6p0esRUfNxaHqY9kJlrJe	t	t	\N	2026-05-19 16:19:50.612412
9	4	ana.hernandez@gmail.com	$2a$06$qebzph9JrScSeXJqTHrPLObi.mcKhQnEs70f8DiNdvMgijao4KmSO	t	t	\N	2026-05-19 16:19:50.612412
10	5	jorge.ramirez@gmail.com	$2a$06$zTGjaduPpR3HCiWjpYvqrOJzMiCqAEZl5gO.XQfl3.UGvyglNB.aO	t	t	\N	2026-05-19 16:19:50.612412
11	6	laura.torres@gmail.com	$2a$06$F3jAafSF8LfL9L0ocZqs2Odsk.aaVj8uI7doYAKCmeSGRP3AYrpsC	t	t	\N	2026-05-19 16:19:50.612412
12	7	pedro.flores@gmail.com	$2a$06$oEZkDTDlPu0jITJfqbwo1ueK1Mw/haPlpE2KB6O36hqj13JXONNHm	t	t	\N	2026-05-19 16:19:50.612412
13	8	elena.rivera@gmail.com	$2a$06$yhzOgv.Zuz21YD925ZUGAOfJZCYxWZFriAspB9LckTRMwGDW2tZh.	t	t	\N	2026-05-19 16:19:50.612412
14	9	miguel.gomez@gmail.com	$2a$06$hC8h3/X24.1sPPDWnyuN/.Iz1ipkf1UQn3DRlSBQpd7JjLqeT8/72	t	t	\N	2026-05-19 16:19:50.612412
15	10	patricia.diaz@gmail.com	$2a$06$kC5ljA7MoHI8ajZ8r5twlulJVXGQVr0Qgi/oWG0W9l0ds6sZIbYFu	t	t	\N	2026-05-19 16:19:50.612412
16	11	fernando.castro@gmail.com	$2a$06$UEdPIoU.F4OIw3F7iroxger0aUv2Oivxz2nDMWfSdZztSKXEN1Tgy	t	t	\N	2026-05-19 16:19:50.612412
17	12	gabriela.ortiz@gmail.com	$2a$06$d7ABpL.QW97IT33lq2cqjucxlOTPT66vPvA4T/u41JnKSglhU9P5G	t	t	\N	2026-05-19 16:19:50.612412
18	14	daniela.ruiz@gmail.com	$2a$06$f5cdssTDh5nHF2ojwm6kQ.yGJwLJvaqHuqetUMTegyC5laUOpS1G2	t	t	\N	2026-05-19 16:19:50.612412
19	15	hugo.navarro@gmail.com	$2a$06$nP2MoAydGnGao8R16TV5HO81M7vPpEcxrRo0sNrJtu0NX0/uUyKYu	t	t	\N	2026-05-19 16:19:50.612412
\.
...
17	21	rodrigo.figueroa@gmail.com	t
22	22	fatima.huerta@gmail.com	t
23	23	luis.bonavena@gmail.com	t
\.
...
21	21	8110000021	Celular	t
26	22	8110000022	Celular	t
27	23	8110000024	Celular	t
\.
...
21	Rodrigo	Figueroa	RODF395928AJVJMV20	\N	\N	\N
22	Fatima	Huerta	FATH1284868VNCLQ22	\N	\N	\N
23	Luis	Bonavena	LUIB20459684JVLL04	\N	\N	\N
\.
...
392	23	8	2	5	Transferencia_Salida	15	40	25	7	transfer	Transferencia #7 aceptada	2026-05-19 08:29:53.365064
393	24	8	1	5	Transferencia_Entrada	15	0	15	7	transfer	Transferencia #7 recibida	2026-05-19 08:29:53.365064
394	14	1	1	3	Salida_Aplicacion	1	72	71	100	vaccination_record	\N	2026-05-19 10:35:47.723933
395	15	2	1	3	Salida_Aplicacion	1	8	7	101	vaccination_record	\N	2026-05-19 10:36:22.850544
396	15	2	1	2	Salida_Aplicacion	1	7	6	102	vaccination_record	\N	2026-05-19 12:08:35.074615
397	4	4	1	2	Salida_Aplicacion	1	86	85	103	vaccination_record	\N	2026-05-19 12:09:03.389673
398	6	6	1	2	Salida_Aplicacion	1	72	71	104	vaccination_record	\N	2026-05-19 12:10:33.376225
399	3	3	1	2	Salida_Aplicacion	1	69	68	105	vaccination_record	\N	2026-05-19 12:11:42.3566
400	20	5	1	3	Salida_Aplicacion	1	50	49	106	vaccination_record	\N	2026-05-19 12:36:54.693145
401	13	1	1	3	Salida_Aplicacion	1	99	98	107	vaccination_record	\N	2026-05-19 12:37:22.933707
402	14	1	1	3	Salida_Aplicacion	1	71	70	108	vaccination_record	\N	2026-05-19 14:56:27.638907
403	15	2	1	3	Salida_Aplicacion	1	6	5	109	vaccination_record	\N	2026-05-19 14:57:11.73766
404	3	3	1	3	Salida_Aplicacion	1	68	67	110	vaccination_record	\N	2026-05-19 15:01:53.26926
405	20	5	1	3	Salida_Aplicacion	1	49	48	111	vaccination_record	\N	2026-05-19 15:02:37.342834
406	13	1	1	3	Salida_Aplicacion	1	98	97	112	vaccination_record	\N	2026-05-19 15:15:02.959674
407	2	2	1	3	Salida_Aplicacion	1	108	107	113	vaccination_record	\N	2026-05-19 15:15:53.638881
408	4	4	1	3	Salida_Aplicacion	1	85	84	114	vaccination_record	\N	2026-05-19 15:16:41.357646
409	20	5	1	3	Salida_Aplicacion	1	48	47	115	vaccination_record	\N	2026-05-19 15:17:03.408446
410	13	1	1	5	Transferencia_Salida	40	97	57	8	transfer	Transferencia #8 aceptada	2026-05-19 15:34:36.732696
411	25	1	2	5	Transferencia_Entrada	40	0	40	8	transfer	Transferencia #8 recibida	2026-05-19 15:34:36.732696
\.
...
7	23	8	2	1	15	Recibido	1	5	Stock insuficiente	Buen estado	2026-05-19 08:28:47.841213	2026-05-19 08:29:53.365064
8	13	1	1	2	40	Recibido	5	5	Redistribución por falta de stock en clinica de destino	Buen estado	2026-05-19 15:34:14.370846	2026-05-19 15:34:36.732696
\.
...
1	21	1	\N	Finalizado	\N	1	2026-05-19 09:35:38.427308	2026-05-19 09:35:38.427308	2026-05-19 09:37:27.988028	\N	2026-05-19 09:37:41.160295	1	1	\N	\N	Espontanea	\N	2026-05-19 09:35:38.427308	2026-05-19 09:37:41.160295
696	27	20	2030-04-12	Pendiente	2026-05-19 15:26:49.209163
697	27	21	2030-04-12	Pendiente	2026-05-19 15:26:49.209163
698	27	22	2031-03-12	Pendiente	2026-05-19 15:26:49.209163
699	27	23	2031-04-12	Pendiente	2026-05-19 15:26:49.209163
700	27	24	2032-04-12	Pendiente	2026-05-19 15:26:49.209163
701	27	25	2037-04-12	Pendiente	2026-05-19 15:26:49.209163
702	27	26	2037-10-12	Pendiente	2026-05-19 15:26:49.209163
\.
...
COPY public.patients (patient_id, first_name, last_name, birth_date, blood_type_id, gender, nfc_id, curp, weight_kg, premature, created_at, is_active, deleted_at, updated_at, photo) FROM stdin;
...
15	Ángel	Navarro	2021-04-11	7	M	\N	NAAA210411HNLRVSA6	14.50	f	2026-05-15 14:00:13.419597	t	\N	\N	P15.png
6	Emma	Torres	2018-08-18	6	F	\N	TOEE180818MNLRRSA6	24.30	f	2026-05-15 14:00:13.419597	t	\N	\N	P6.png
26	Ivan	Bonavena	2025-04-01	7	M	\N	IVAB1049683MVSKV02	\N	f	2026-05-19 15:12:36.131014	t	\N	2026-05-19 15:44:11.785654	P26.png
21	Tobías	Figueroa	2026-04-26	3	M	\N	TOBF294960BMSKVL22	\N	f	2026-05-19 08:40:14.335041	t	\N	2026-05-19 09:50:41.533794	\N
8	Camila	Rivera	2020-09-14	8	F	\N	RICC200914MNLRVCA8	19.40	t	2026-05-15 14:00:13.419597	t	\N	2026-05-19 09:53:26.727749	P8.png
20	Miguel	Lopez	2026-01-18	1	M	\N	MIGL305919MNLMTYN0	\N	f	2026-05-18 19:43:45.972245	t	\N	\N	P20.png
12	Regina	Ortiz	2019-10-16	4	F	\N	OARR191016MNLRRGA3	21.80	f	2026-05-15 14:00:13.419597	t	\N	\N	P12.jpeg
18	Mariana	Olvera	2026-03-10	1	F	\N	MARO401930MNLRVCA8	\N	f	2026-05-17 00:33:11.833123	t	\N	\N	P18.jpeg
13	Daniel	Morales	2020-07-01	5	M	\N	MODD200701HNLRNTA4	17.90	t	2026-05-15 14:00:13.419597	t	\N	\N	P13.png
9	Leonardo	Gómez	2017-12-30	1	M	\N	GOAL171230HNLRMDA9	29.10	f	2026-05-15 14:00:13.419597	t	\N	\N	P9.png
5	Lucas	Ramírez	2021-06-12	5	M	\N	RALL210612HNLRMNA5	16.20	f	2026-05-15 14:00:13.419597	t	\N	\N	P5.png
11	Emiliano	Castro	2018-05-03	3	M	\N	CAEE180503HNLRMSA2	23.60	f	2026-05-15 14:00:13.419597	t	\N	\N	P11.png
10	Renata	Díaz	2021-02-25	2	F	\N	DIRR210225MNLRZEA1	15.70	f	2026-05-15 14:00:13.419597	t	\N	\N	P10.png
22	Eugenio	Rodríguez	2026-03-11	2	M	\N	EUGP194858MSLAL019	\N	f	2026-05-19 10:32:38.871803	t	\N	2026-05-19 10:40:01.230645	\N
25	Patricia	Huerta	2025-02-02	1	F	\N	PATH294970MAJVKD01	\N	f	2026-05-19 14:54:26.19645	t	\N	\N	P25.png
23	Raúl	Morales	2026-05-02	7	M	\N	RAUM1228558MSLAL35	\N	f	2026-05-19 12:30:11.343401	t	\N	2026-05-19 15:06:37.766272	P23.png
24	Adrian	Vazquez	2025-01-03	7	M	\N	ADRV1841910CLAL019	\N	f	2026-05-19 12:34:16.998522	t	\N	2026-05-19 15:06:48.589278	\N
27	Sebastian	Bonavena	2026-04-12	1	M	\N	SEBB1029475JVMVOS0	8.40	f	2026-05-19 15:26:49.209163	t	\N	\N	\N
\.
...
COPY public.scheme_completion_alerts (alert_id, patient_id, scheme_dose_id, due_date, status, notified_at, alert_type, read_at, schedule_id, created_at) FROM stdin;
\.
...
100	22	1	3	1	14	1	2026-05-19	\N	\N	\N	f	2026-05-19 10:35:47.723933	547	\N
101	22	2	3	1	15	2	2026-05-19	4	\N	\N	f	2026-05-19 10:36:22.850544	548	\N
102	6	2	2	1	15	2	2026-05-19	\N	\N	\N	f	2026-05-19 12:08:35.074615	21	\N
103	6	4	2	1	4	4	2026-05-19	\N	\N	\N	f	2026-05-19 12:09:03.389673	51	\N
104	6	6	2	1	6	9	2026-05-19	\N	\N	\N	f	2026-05-19 12:10:33.376225	126	\N
105	6	3	2	1	3	3	2026-05-19	\N	\N	\N	f	2026-05-19 12:11:42.3566	36	\N
106	24	5	3	1	20	5	2026-05-19	\N	\N	\N	f	2026-05-19 12:36:54.693145	603	\N
107	24	1	3	1	13	1	2026-05-19	\N	\N	\N	f	2026-05-19 12:37:22.933707	599	\N
108	25	1	3	1	14	1	2026-05-19	3	\N	\N	f	2026-05-19 14:56:27.638907	625	\N
109	25	2	3	1	15	2	2026-05-19	4	\N	\N	f	2026-05-19 14:57:11.73766	626	\N
110	4	3	3	1	3	3	2026-05-19	\N	24	\N	f	2026-05-19 15:01:53.26926	34	\N
111	4	5	3	1	20	5	2026-05-19	\N	24	\N	f	2026-05-19 15:02:37.342834	64	\N
112	26	1	3	1	13	1	2026-05-19	\N	\N	\N	f	2026-05-19 15:15:02.959674	651	\N
113	26	2	3	1	2	2	2026-05-19	\N	\N	\N	f	2026-05-19 15:15:53.638881	652	\N
114	26	4	3	1	4	4	2026-05-19	\N	\N	\N	f	2026-05-19 15:16:41.357646	654	\N
115	26	5	3	1	20	5	2026-05-19	\N	\N	\N	f	2026-05-19 15:17:03.408446	655	\N
\.
...
24	8	1	SRP-2026-S002	15	15	2026-08-19	2026-05-19	t	Disponible
14	1	1	LOTE-HB-2024-001	100	70	2026-11-13	2026-04-17	t	Disponible
15	2	1	LOTE-BCG-2024-002	50	5	2026-06-11	2026-03-18	t	Disponible
3	3	1	PENT-2025-C001	100	67	2026-08-31	2025-01-15	t	Disponible
2	2	1	HEPB-2025-B001	150	107	2026-10-15	2025-01-05	t	Disponible
4	4	1	HEPBS-2025-D001	100	84	2026-10-15	2025-01-15	t	Disponible
20	5	1	ROT-2026-R002	50	47	2026-11-17	2026-05-17	t	Disponible
13	1	1	BCG-2026-B002	100	57	2026-07-15	2026-05-15	t	Disponible
26	7	2	INF-2026-I003	30	30	2026-08-18	2026-05-19	t	Disponible
...
23	8	2	SRP-2026-S002	40	25	2026-08-19	2026-05-19	t	Disponible
6	6	1	NEUM-2025-F001	120	71	2026-12-31	2025-01-20	t	Disponible
25	1	2	BCG-2026-B002	40	40	2026-07-15	2026-05-19	t	Disponible
...
2	1	\N	\N	En espera	En consulta	2026-05-19 09:37:27.988028	1	\N	NFC m‚dico - inicia consulta
1	1	\N	\N	\N	En espera	2026-05-19 09:35:38.427308	1	\N	Check-in - ingresa a sala de espera
3	1	\N	\N	En consulta	Finalizado	2026-05-19 09:37:41.160295	1	\N	Check-out NFC
5	2	\N	\N	En espera	En consulta	2026-05-19 12:46:21.387893	1	\N	NFC m‚dico - inicia consulta
4	2	\N	\N	\N	En espera	2026-05-19 12:46:05.98419	1	\N	Check-in - ingresa a sala de espera
6	2	\N	\N	En consulta	Finalizado	2026-05-19 12:46:33.607594	1	\N	Check-out NFC
8	3	\N	\N	En espera	En consulta	2026-05-19 15:28:30.466336	4	\N	NFC m‚dico - inicia consulta
7	3	\N	\N	\N	En espera	2026-05-19 15:28:20.037188	4	\N	Check-in - ingresa a sala de espera
9	3	\N	\N	En consulta	Finalizado	2026-05-19 15:28:40.717508	4	\N	Check-out NFC
\.
...
SELECT pg_catalog.setval('public.audit_log_audit_id_seq', 196, true);
...
SELECT pg_catalog.setval('public.guardian_accounts_guardian_account_id_seq', 19, true);
...
SELECT pg_catalog.setval('public.guardian_emails_email_id_seq', 25, true);
...
SELECT pg_catalog.setval('public.guardian_phones_phone_id_seq', 29, true);
...
SELECT pg_catalog.setval('public.guardians_guardian_id_seq', 23, true);
...
SELECT pg_catalog.setval('public.inventory_movements_movement_id_seq', 411, true);
...
SELECT pg_catalog.setval('public.inventory_transfers_transfer_id_seq', 8, true);
...
SELECT pg_catalog.setval('public.nfc_cards_nfc_card_id_seq', 8, true);
...
SELECT pg_catalog.setval('public.nfc_scan_events_scan_event_id_seq', 9, true);
...
SELECT pg_catalog.setval('public.patient_clinic_visits_visit_id_seq', 3, true);
...
SELECT pg_catalog.setval('public.patient_guardian_relations_relation_id_seq', 27, true);
...
SELECT pg_catalog.setval('public.patient_vaccine_schedule_schedule_id_seq', 702, true);
...
SELECT pg_catalog.setval('public.patients_patient_id_seq', 27, true);
...
SELECT pg_catalog.setval('public.vaccination_records_record_id_seq', 115, true);
...
SELECT pg_catalog.setval('public.vaccine_lots_lot_id_seq', 26, true);
...
SELECT pg_catalog.setval('public.visit_area_movements_movement_id_seq', 9, true);
...
ALTER TABLE ONLY public.patients
    ADD CONSTRAINT patients_nfc_token_key UNIQUE (nfc_id);
...
CREATE INDEX idx_patients_nfc_token ON public.patients USING btree (nfc_id);
...
\unrestrict MTI0IbsfvR6mBlr5ySsyB4RUWlksL94H0dzAoSoIuhE6fRq8cjQdyfp8lns3uY2
