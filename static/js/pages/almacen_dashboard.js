  ...
  if (typeof Highcharts !== 'undefined') {
    ...
    if (dataEl) {
      ...
      try {
        ...
        console.error('[almacen_dashboard] Error al parsear datos de gráfica:', e);
        lotesCriticos = [];
      }

      if (Array.isArray(lotesCriticos) && lotesCriticos.length > 0 &&
          document.getElementById('chartConsumoSemana')) {
        ...
        var stockData = lotesCriticos.map(function (l) {
          ...
          return {
            y: qty,
            color: qty <= 5 ? colorError : colorPrimary,
          };
        });
